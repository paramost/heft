...
**Two teaches, no tutorial.** 1.9.0. Dan's worry after the first live days: a new player does
...
wedge, thin at the light end and thick at the heavy end (1.9.1), which Dan caught asserting a
ratio — each step heavier by the same amount — that the game does not make; so a bar of even
thickness whose colour runs from dim to the weights' own orange, labelled at its ends (1.9.2):
it says which way is heavier and nothing about by how much; and the prompt names the swap — "That's a swap. Fewer is better." — in

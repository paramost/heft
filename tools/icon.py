...
Render the favicon's design as a PNG: the apple-touch-icon Messages shows beside a link.
    ...
    python3 tools/icon.py icon-1.png

The favicon in index.html is an inline SVG - a 32-unit rounded square in the header blue with
the H of the wordmark in the ink colour - and link previews ignore inline SVG. This draws the
same thing at 180x180, full bleed (iOS rounds the corners itself), with 4x supersampling on the
letter's edges. Standard library only. Change the design, change the filename: iMessage caches a
preview per URL.
"""
